                ...
                with tf.variable_scope(extra_layer_scope, reuse=reuse):
                    if layer_type == 'dense':
                        ...
                        with tf.variable_scope(layer_name, reuse=reuse):
                            ...
                            # Use tf.reduce_prod for dynamic parts if necessary, or assume static for non-batch/time
                            if any(isinstance(s, tf.Tensor) for s in inp0_shape[2:]):
                                last_dim = tf.reduce_prod(inp0_shape[2:])
                                inp[nodes_out[0]] = tf.reshape(inp0, shape=tf.concat([inp0_shape[0:2], [last_dim]], axis=0))
                            else:
