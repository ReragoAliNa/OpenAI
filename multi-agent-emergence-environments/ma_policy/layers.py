...
def entity_concat(inps):
    ...
    with tf.variable_scope('concat_entities'):
        ...
        shapes = [shape_list(_x) for _x in inps]
        static_shapes = [_x.get_shape().as_list() for _x in inps]
        for s in static_shapes:
            if s[-1] is not None and static_shapes[0][-1] is not None:
                assert s[-1] == static_shapes[0][-1], \
                    f"Some entities don't have the same outer or inner dimensions {static_shapes}"
        ...
        for inp, mask in zip(inps, masks):
            if mask is None:
                ...
                if len(inp_shape) == 4:  # this is an entity tensor
                    new_masks.append(tf.ones(tf.stack(inp_shape[:3])))
                elif len(inp_shape) == 3:  # this is a pooled or main tensor. Set NE (outer dimension) to 1
                    new_masks.append(tf.ones(tf.stack(inp_shape[:2] + [1])))
            else:
    ...
    with tf.variable_scope(scope, reuse=reuse):
        ...
        if mask is not None:
            with tf.variable_scope('expand_mask'):
                ...
                inp_shape = inp.get_shape().as_list()[:3]
                for m_dim, i_dim in zip(mask_shape, inp_shape):
                    if m_dim is not None and i_dim is not None:
                        assert m_dim == i_dim, \
                            f"Mask and input should have the same first 3 dimensions. {mask_shape} -- {inp_shape}"
                entity_mask = mask
